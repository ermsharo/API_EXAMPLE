...
router.post('/auth/singup', async (req, res) => {
	...
	if (password != passwordConfirm) return res.status(400).send('Password check is diferent from password');
	...
	if (getUserByUsername === null) {
		...
		if (getEmailByEmailtext === null) {
			...
			return res.status(400).send('Alredy exist a acoount with this email adrress');
		}
	} else {
...
router.post('/auth/singin', async (req, res) => {
	...
	if (userByMail.email == email && bcrypt.compareSync(password, userByMail.password)) {
